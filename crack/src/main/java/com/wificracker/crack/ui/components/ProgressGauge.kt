...
fun ProgressGauge(progress: CrackProgress, modifier: Modifier = Modifier) {
    Card(modifier = modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Column { Text("Speed", style = MaterialTheme.typography.labelSmall); Text("${progress.keysPerSecond} keys/s", style = MaterialTheme.typography.bodyMedium) }
                Column(horizontalAlignment = Alignment.CenterHorizontally) { Text("Tested", style = MaterialTheme.typography.labelSmall); Text("${progress.keysTested}", style = MaterialTheme.typography.bodyMedium) }
                Column(horizontalAlignment = Alignment.End) { Text("ETA", style = MaterialTheme.typography.labelSmall); Text(progress.eta.ifBlank { "--" }, style = MaterialTheme.typography.bodyMedium) }
            }
